...
omega_setpoint_ = (force_setpoint_ * wheel_radius_) / motor_stall_torque_ * omega_no_load_;            RCLCPP_DEBUG(get_logger(), "Received force setpoint: %.3f N, calculated torque setpoint: %.3f N-m, resulting in omega setpoint: %.2f rad/s", force_setpoint_, torque_setpoint, omega_setpoint_);    
